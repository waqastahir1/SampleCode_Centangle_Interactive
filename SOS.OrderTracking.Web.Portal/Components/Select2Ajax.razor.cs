...
namespace SOS.OrderTracking.Web.Portal.Components
{
    public partial class Select2Ajax<TValue> : InputBase<TValue>
    {
        ...
        public void ChangeFromParent(string text, int value)
        {
            try
            {
                //if (Nullable.GetUnderlyingType(typeof(TValue)) != null) Value = (TValue)Convert.ChangeType(value, Nullable.GetUnderlyingType(typeof(TValue)));
                //else Value = (TValue)Convert.ChangeType(value, typeof(TValue));
                JSRuntime.InvokeVoidAsync("selectAjax2Component.init", Id, ModalId, AjaxUrl, value, text, Id1, Id2);
            }
            catch (Exception ex)
            {
                logger.LogError(ex.ToString());
